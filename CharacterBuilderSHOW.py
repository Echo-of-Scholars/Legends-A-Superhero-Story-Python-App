...
from PyQt5.QtWidgets import QMainWindow, QInputDialog, QMessageBox, QWidget, QVBoxLayout, QFileDialog, QComboBox, \
    QSpinBox, QLabel, QLineEdit, QPushButton, QTextEdit, QCheckBox
from calculateLvls import calculateLvls, calculateBasedEvents
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        icon = QIcon(r'H:\Programs\QTDesignWorks\Legends\legendsicon.ico')
        ...
        self.RanGenBut.setGeometry(QtCore.QRect(40, 75, 75, 23))
        self.RanGenBut.setObjectName("RanGenBut")
        self.RanGenBut.clicked.connect(lambda: self.comingsoon())
        self.diceBut = QtWidgets.QPushButton(self.centralwidget)
        self.diceBut.setGeometry(QtCore.QRect(127, 75, 75, 23))
        self.diceBut.setObjectName("diceBut")
        self.diceBut.clicked.connect(lambda: self.diceRoller())
        ...
        self.lvlSPN.valueChanged.connect(lambda newlvl: calculateBasedEvents(self, newlvl))
        ...
        self.xpSPN.valueChanged.connect(lambda: calculateLvls(self))
        ...
        MainWindow.setTabOrder(self.RanGenBut, self.diceBut)
        MainWindow.setTabOrder(self.diceBut, self.ExitBut)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.RanGenBut.setText(_translate("MainWindow", "Random"))
        ...
        self.diceBut.setText(_translate("MainWindow", "Dice Roller"))
    ...
    # TODO: Track skill points? All settings work as intended until feedback is given.
    def pwrSetting(self):
        if self.pwrSettingBox.currentText() == "Low Power Setting":
            ...
            while True:
                ...
                if ok1 and ok2 and ok3 and ok4 and ok5:
                    ...
                    msg = QMessageBox()
                    msg.setText("Please fill out all fields before continuing.")
                    msg.exec_()
                    return

    def getPwrSetting(self):
        # I hate that I can't figure out a better way. Gets HP/SP setting from Power Setting
        return getattr(self, 'curPwrSet', None)
    ...
    # TODO: Works, may need a new coat of paint. Aka css
    def powerlisting(self):
        ...
        if power and power not in [self.pwrArea.item(i).text() for i in range(self.pwrArea.count())]:
            if self.pwrPTs >= pwrpointcost:
                ...
                msg.setText("Not enough Power or Training Points.")
                msg.exec_()
                return
    ...
    def HPSP(self, agitot, SPDtot, inttot, STRtot, Hitval, STRval, STAtot, setVal):
        ...
        if self.unpowAGLedit and self.unpowSPDedit \
                and self.unpowINTedit and self.unpowSTRedit:
        ...
        if self.unpowSTAedit and self.unpowSTRedit:
    ...
    def Ini(self, agitot, AGIinival, SPDtot, SPDinival):
        ...
        if self.unpowSPDedit and self.unpowAGLedit:
            if agitot > SPDtot:
                inival = AGIinival
    ...
    def Def(self, agitot, AGIdef, sentot, senDef):
        ...
        if self.unpowAGLedit and self.unpowSENedit:
            if agitot > sentot:
                defval = AGIdef
                deftotal = defe + defval
                self.defSCRSPN.setValue(deftotal)
            elif sentot < agitot:
                defval = senDef
                deftotal = defe + defval
                self.defSCRSPN.setValue(deftotal)
    ...
    def Acc(self, sentot, senAcc, inttot, intAcc):
        ...
        if self.unpowSENedit and self.unpowINTedit:
            if inttot > sentot:
                accval = intAcc
                accTot = acc + accval
                self.accSCRSPN.setValue(accTot)
            elif sentot < inttot:
                accval = senAcc
                accTot = acc + accval
                self.accSCRSPN.setValue(accTot)
    ...
    def wrapperHPSP(self):
        ...
        curPwrSet= self.getPwrSetting()
        if curPwrSet is None:
            curPwrSet = 0
        self.HPSP(agitot, SPDtot, inttot, STRtot, Hitval, STRval, STAtot, curPwrSet)
    ...
    def skillBoxes(self):
        self.unpowskillboxes = [self.unPowAGLSPN,
                      self.unpowCHASPN,
                      self.unpowINTSPN,
                      self.unpowSENSPN,
                      self.unpowSPDSPN,
        ...
        for skillbox in self.unpowskillboxes:
            skillbox.valueChanged(self.trackSkillPoints)

        skillvalue = sum(skillbox.value() for skillbox in self.unpowskillboxes)
        return skillvalue
    ...
    def trackSkillPoints(self):
        usedPts = self.skillboxes()
        initpool = 63

        Ptsused = max(0, usedPts - initpool)

        switchSklPts = max(0, self.sklPTs - Ptsused)

        self.sklPTs = switchSklPts

        if self.sklPTs > 0 and switchSklPts == 0 and Ptsused > 0:
            # Not enough Skill or Training points to add the power
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setWindowTitle("Not Enough!")
            msg.setText("Not enough Skill or Training Points.")
            msg.exec_()
    ...
    # Only three away from ditching this.
    def comingsoon(self):
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Warning)
...
class DiceRoller(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.rollDescriptionLineEdit = QLineEdit()
        self.rollDescriptionLineEdit.setPlaceholderText("Roll Description (e.g., Punch/Power/Skill)")
        layout.addWidget(QLabel("Description:"))
        layout.addWidget(self.rollDescriptionLineEdit)

        # Number of dice
        self.numDiceSpinBox = QSpinBox()
        self.numDiceSpinBox.setMinimum(1)
        layout.addWidget(QLabel("Number of Dice:"))
        layout.addWidget(self.numDiceSpinBox)

        # Type of dice
        self.diceTypeComboBox = QComboBox()
        self.diceTypeComboBox.addItems(["d2", "d4", "d6", "d8", "d10", "d12", "d20", "d100"])
        layout.addWidget(QLabel("Type of Dice:"))
        layout.addWidget(self.diceTypeComboBox)

        # Modifier input
        self.modifierLineEdit = QLineEdit()
        self.modifierLineEdit.setPlaceholderText("Modifier (optional)")
        layout.addWidget(QLabel("Modifier:"))
        layout.addWidget(self.modifierLineEdit)

        # Checkbox for showing each roll or total
        self.showEachRollCheckBox = QCheckBox("Show Each Roll")
        layout.addWidget(self.showEachRollCheckBox)
        ...
        # Results display
        self.resultsLineEdit = QLineEdit()
        self.resultsLineEdit.setReadOnly(True)
        layout.addWidget(QLabel("Results:"))
        layout.addWidget(self.resultsLineEdit)

        # Roll record
        self.rollRecordTextEdit = QTextEdit()
        self.rollRecordTextEdit.setReadOnly(True)
        layout.addWidget(QLabel("Roll Record:"))
        layout.addWidget(self.rollRecordTextEdit)

        self.modifierLineEdit.returnPressed.connect(lambda: self.rollDice())

    def validateMod(self, text):
        try:
            int(text)
            return True
        except ValueError:
    ...
    @pyqtSlot()
    def rollDice(self):
        description = self.rollDescriptionLineEdit.text()
        numDice = self.numDiceSpinBox.value()
        diceType = self.diceTypeComboBox.currentText()
        maxRoll = int(diceType[1:])  # Get the numerical part of diceType, e.g., "d20" -> 20
        rolls = [random.randint(1, maxRoll) for _ in range(numDice)]

        # Safe conversion of modifier text to integer
        modtext = self.modifierLineEdit.text().strip()
        try:
            modifier = int(modtext or "0")
        except ValueError:
            modifier = 0  # Default to 0 if conversion fails

        total = sum(rolls) + modifier

        # Handle special messages for min and max rolls
        critMessage = ""
        if maxRoll != 2:  # Exclude d2 from special messages
            if all(roll == 1 for roll in rolls):
                critMessage = " - Crit Fail!"
            elif all(roll == maxRoll for roll in rolls):
                critMessage = " - Crit Success!"

        resultsText = f"{description}: " if description else ""
        if self.showEachRollCheckBox.isChecked():
            resultsText += ", ".join(map(str, rolls)) + (f" + {modifier}" if modifier else "") + f" = {total}"
        else:
            resultsText += str(total)
        resultsText += critMessage
        self.resultsLineEdit.setText(resultsText)

        # Update roll record
        rollRecordEntry = f"{description}: {numDice}{diceType}{'+' if modifier > 0 else ''}{modifier if modifier else ''} = {total}{critMessage}\n"
        self.rollRecordTextEdit.append(rollRecordEntry)
